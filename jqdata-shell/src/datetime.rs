...
impl DatetimeProcessor {
    ...
    pub(crate) fn next(&self, ts: &str) -> Result<String> {
        let curr_dt = parse_ts_from_str(ts)?;
        let next_dt = curr_dt
    ...
    pub(crate) fn end_of_today(&self) -> String {
        format!(
            "{} 23:59",
            Local::today().naive_local().format(DATETIME_FORMAT)
        )
    }
...
impl DatetimeRange {
    pub(crate) fn new(min: &str, max: &str) -> Result<Self> {
        let min = parse_ts_from_str(min)?;
        let max = parse_ts_from_str(max)?;
        if min > max {
...
pub fn parse_ts_from_str(s: &str) -> Result<NaiveDateTime> {
    match s.len() {
        10 => {
